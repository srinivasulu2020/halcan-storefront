             ...
             <div className="grid gap-12 lg:grid-cols-3 lg:gap-8">
                ...
                <div className="lg:col-span-2">
                  ...
                  <button 
                    ...
                    <div className={`group-focus-visible:outline-primary-950 dark:group-focus-visible:outline-primary-700 flex items-center justify-between rounded-3xl group-focus-visible:outline group-focus-visible:outline-2 group-focus-visible:outline-offset-2 px-6 ${(open)&&(service.id===openid)?'bg-slate-200':''}  ${(open)&&(service.id===openid)?'p-4':''}`}>
                    <div className="grid grid-col gap-6">
                       <span className="font-medium" > {service.name}</span>
                          
                        {
                           (open)&&(service.id===openid)?<span key={service.id}  onChange={(e:any)=>setopenid(e.target.value)}>{service.description } </span>:null   
                        }
                    </div>
